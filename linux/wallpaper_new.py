...
cloudinary.config(
  cloud_name = "deyhhcqns",
  api_key = "548645347158887",
  api_secret = "r51YFWvoYqxouUFPI1mmZcpqZZI"
)
...
def download_img(year, month, day, hour, minute, second):
    ...
    url = "http://res.cloudinary.com/deyhhcqns/image/upload/v1474642375/earth/%s/%s/%s/%s_%s_%s.png" \
          % (year, month, day, hour, minute, second)

    url_last = "http://res.cloudinary.com/deyhhcqns/image/upload/v1474642375/earth/%s/%s/%s/%s_%s_%s.png" \
               % (year, month, day, hour, str(int(minute)-10) , second)
    ...
    try:
        ...
        print "download last picture..."
        request_img__last = urllib2.Request(url_last)
        response_img = urllib2.urlopen(request_img__last)
        data_img = response_img.read()
        picname = os.path.join(os.getcwd(), "Earth.png")  # pic path under the script dir
        with open(picname, 'wb') as fp:
            fp.write(data_img)
